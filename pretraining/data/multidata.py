...
class MultiDataLoader:
    ...
    def __init__(
        ...
        self.loaders_names = loaders_names
        assert len(loaders_names) == self.num_datasets
        self.datasets_sampled_counts = [0] * len(loaders_names)
    ...
    def change_dataloader(self):
        ...
        if wandb.run is not None and self.split_name == 'train':  # https://github.com/wandb/wandb/issues/3551
            self.datasets_sampled_counts[self.current_index] += 1
            wandb.log(dict(zip(self.loaders_names, self.datasets_sampled_counts)), commit=False)
...
class MultiDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        for datamodule in self.datamodules:
            print(f"{datetime.now()} Setting up datamodule: {datamodule.name}")
            datamodule.setup(stage)

    def prepare_data(self):
        for datamodule in self.datamodules:
            print(f"{datetime.now()} Preparing data for datamodule: {datamodule.name}")
    ...
    def _build_multi_dataloader(self, split="train"):
        dataloaders = []
        for datamodule in self.datamodules:
            print(f"{datetime.now()} Building {split} dataloader for datamodule: {datamodule.name}")
            dataloaders.append(getattr(datamodule, f"{split}_dataloader")())

        return MultiDataLoader(split_name=split,
                               loaders_names=[l.name for l in self.datamodules],
                               loaders=dataloaders,
