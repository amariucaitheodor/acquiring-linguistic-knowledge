...
from assets.plots.thesis.utils import BLIMP_CATEGORIES, get_label_map, label_group_bar_table
    ...
    for text_perc in [1, 10]:
        for vision_perc in [0, 1, 10, 100]:
            for cat, formatted_cat in BLIMP_CATEGORIES.items():
                ...
                for vision_type in ['No Vision', 'Slight Vision', 'Full Vision', 'Extra (400K) Vision',
                                    'Extra (4M) Vision']:
...
def plot():
    ...
    if PLOT_TYPE == 'bar':
        ...
        sns.heatmap(plot_df, ax=ax,
                    ...
                    cmap=sns.color_palette('RdYlGn', 5), mask=(plot_df == 0),
